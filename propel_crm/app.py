...
def gerar_vitorias_semana(cliente_id: int, semana: int):
    ...
    proximos = f"\n\n🎯 *Próximos passos para a Semana {semana + 1 if semana < 4 else 1}:*\n" + "\n".join([f"🔜 {t}" for t in pendentes]) if pendentes else ""

    texto = (
        f"Fala, {cliente['responsavel_nome']}! Passando com o resumo oficial das nossas *Vitórias da Semana {semana}* na {cliente['nome_empresa']}:\n\n"
        f"{itens_formatados}"
        f"{proximos}\n\n"
...
# ── ROTA DA CENTRAL DE AUDITORIA DIGITAL 360 ──────────────────────────────

@app.get("/auditoria", response_class=HTMLResponse)
def auditoria_view(request: Request):
    user = request.cookies.get("session_user") or "Especialista Propel"
    return templates.TemplateResponse(request=request, name="auditoria.html", context={"user": user})
