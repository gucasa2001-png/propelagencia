...
def cadastrar_novo_cliente_completo(dados: dict) -> int:
    ...
    1. Registra os dados da empresa e parâmetros de verba/CPA/logo.
    ...
    verba_mensal = float(dados.get("verba_mensal", 1500.0))
    ...
    with get_db() as conn:
        ...
        INSERT INTO clientes_trafego (
            ...
            verba_mensal, meta_cpa, status, data_inicio, logo_url, cor_primaria
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, 'ativo', ?, ?, ?)
        """, (
            nome_empresa, segmento, responsavel_nome, whatsapp, magic_token,
            verba_mensal, meta_cpa, agora_iso, logo_url, cor_primaria
        ))
        ...
        for semana, titulo, freq, cor in ENTREGAS_TEMPLATE:
            ...
            INSERT INTO roadmap_entregas (cliente_id, semana, titulo, frequencia, frequencia_cor, status)
            VALUES (?, ?, ?, ?, ?, 'pendente')
            """, (cliente_id, semana, titulo, freq, cor))
        ...
        VALUES (?, ?, 'campanha', ?)
        """, (cliente_id, f"Cliente {nome_empresa} cadastrado no Propel CRM com Roadmap de 4 Semanas ativado.", agora_formatado))
