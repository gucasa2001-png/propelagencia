...
def init_db():
    with get_db() as conn:
        ...
        cursor.execute("PRAGMA table_info(clientes_trafego)")
        cols = [row["name"] for row in cursor.fetchall()]
        if "logo_url" not in cols:
            cursor.execute("ALTER TABLE clientes_trafego ADD COLUMN logo_url TEXT DEFAULT ''")
        if "cor_primaria" not in cols:
            cursor.execute("ALTER TABLE clientes_trafego ADD COLUMN cor_primaria TEXT DEFAULT '#1E40AF'")
        if "plano_projeto" not in cols:
            cursor.execute("ALTER TABLE clientes_trafego ADD COLUMN plano_projeto TEXT DEFAULT 'lite'")
        if "whatsapp_grupo" not in cols:
            cursor.execute("ALTER TABLE clientes_trafego ADD COLUMN whatsapp_grupo TEXT DEFAULT ''")

        # Tabela de Configuração Customizada das Vitórias da Semana
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS vitorias_semana_config (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            cliente_id INTEGER NOT NULL,
            semana INTEGER NOT NULL,
            saudacao TEXT DEFAULT '',
            destaques_custom TEXT DEFAULT '',
        ...
        cursor.execute("PRAGMA table_info(roadmap_entregas)")
        rm_cols = [row["name"] for row in cursor.fetchall()]
        if "projeto_tipo" not in rm_cols:
            cursor.execute("ALTER TABLE roadmap_entregas ADD COLUMN projeto_tipo TEXT DEFAULT 'lite'")
        if "fase_implementacao" not in rm_cols:
            cursor.execute("ALTER TABLE roadmap_entregas ADD COLUMN fase_implementacao TEXT DEFAULT 'Fase I – Piloto'")
        ...
        # 8. Central de Auditoria Digital 360 (Instagram, GMB e Site)
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS auditorias_digitais (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            lead_id INTEGER,
            cliente_trafego_id INTEGER,
            nome_empresa TEXT NOT NULL,
            auditor TEXT DEFAULT 'Especialista Propel',
            score_geral REAL DEFAULT 0.0,
            score_instagram REAL DEFAULT 0.0,
            score_google REAL DEFAULT 0.0,
            score_site REAL DEFAULT 0.0,
            dados_json TEXT NOT NULL,
            data_criacao TEXT NOT NULL,
            data_atualizacao TEXT NOT NULL,
            FOREIGN KEY (lead_id) REFERENCES leads(id),
            FOREIGN KEY (cliente_trafego_id) REFERENCES clientes_trafego(id)
        )
        """)
...
PROJETOS_ESCOPO = {
    "lite": {
        "id": "lite",
        "nome": "Projeto Lite Propel",
        "valor": 1500.0,
        "canais_desc": "1 canal de anúncio (Meta Ads ou Google Ads)",
        "badge_cor": "bg-blue-100 text-blue-800 border-blue-200",
        "canais_qtd": 1,
        "growth_canais": "1 canal prioritário",
        "entregaveis": [
            "Setup completo da conta de anúncios & Business Manager",
            "Criação de copys persuasivas, links rastreados e headlines",
            "Assessoria comercial, alinhamento de funil e CRM completo",
            "Consultoria e checklist de perfil do Instagram",
            "Criação de todas as artes estáticas dos anúncios",
            "1 canal de anúncio ativo e otimizado (Meta ou Google)",
            "Consultoria para planejamento estratégico de marketing",
            "Growth Marketing em canal de conversão prioritário",
            "Melhoria contínua de páginas e conversão no WhatsApp",
            "Automação de processos e mensagens pré-definidas"
        ]
    },
    "soft": {
        "id": "soft",
        "nome": "Projeto Soft Propel",
        "valor": 2500.0,
        "canais_desc": "2 canais de anúncios (Meta Ads + Google Ads / Waze)",
        "badge_cor": "bg-emerald-100 text-emerald-800 border-emerald-200",
        "canais_qtd": 2,
        "growth_canais": "6/19 canais de growth inexplorados",
        "entregaveis": [
            "Todos os entregáveis do Projeto Lite inclusos",
            "2 canais de anúncios simultâneos ativos (ex: Meta + Google)",
            "Produção contínua de criativos em vídeo e variações A/B",
            "Growth Marketing explorando 6 canais inexplorados",
            "Otimização avançada de CPA e conversão no WhatsApp",
            "Relatórios quinzenais de inteligência de tráfego"
        ]
    },
    "gold": {
        "id": "gold",
        "nome": "Projeto Gold Propel",
        "valor": 4000.0,
        "canais_desc": "Multicanal Completo (Meta, Google, Waze, YouTube, TikTok, Taboola)",
        "badge_cor": "bg-amber-100 text-amber-800 border-amber-200",
        "canais_qtd": "Multicanal",
        "growth_canais": "19/19 canais de growth inexplorados",
        "entregaveis": [
            "Todos os entregáveis dos Projetos Lite e Soft inclusos",
            "Multicanal completo com escala agressiva de orçamento",
            "Growth Marketing full-stack (19 canais inexplorados)",
            "Automações avançadas e integração CRM em tempo real",
            "Consultoria estratégica 1x1 prioritária com a diretoria",
            "Auditoria Digital 360 e relatórios executivos sob demanda"
        ]
    }
}

FASES_IMPLEMENTACAO = [
    {"fase": 1, "nome": "Fase I – Projeto Piloto", "desc": "Setup de contas, kickoff, alinhamento e primeiros criativos", "cor": "blue"},
    {"fase": 2, "nome": "Fase II – Validação", "desc": "Veiculação de testes, validação de públicos, mensagens e CPA", "cor": "emerald"},
    {"fase": 3, "nome": "Fase III – Escala", "desc": "Aumento de verba, tração em criativos vencedores e esteira comercial", "cor": "amber"},
    {"fase": 4, "nome": "Fase IV – Expansão", "desc": "Expansão de plataformas, novos canais de mídia e growth marketing", "cor": "purple"}
]


def cadastrar_novo_cliente_completo(dados: dict) -> int:
    """
    Cadastra um novo cliente de tráfego pago de forma completa:
    1. Registra os dados da empresa e parâmetros de verba/CPA/logo/plano.
    2. Popula automaticamente o ciclo de 17 entregas do Roadmap de 4 Semanas.
    3. Cria a linha inicial de métricas de tráfego.
    4. Registra a primeira atividade na Linha do Tempo da agência.
    """
    agora_iso = datetime.now().strftime("%Y-%m-%d")
    agora_formatado = datetime.now().strftime("%d/%m/%Y às %H:%M")

    nome_empresa = dados.get("nome_empresa", "").strip()
    segmento = dados.get("segmento", "").strip()
    responsavel_nome = dados.get("responsavel_nome", "").strip()
    whatsapp = dados.get("whatsapp", "").strip()
    magic_token = dados.get("magic_token", "").strip().lower()
    plano_projeto = dados.get("plano_projeto", "lite").strip().lower()
    if plano_projeto not in ("lite", "soft", "gold"):
        plano_projeto = "lite"

    # Se a verba não foi especificada manualmente, adotar o valor padrão do projeto da proposta
    valor_padrao = PROJETOS_ESCOPO.get(plano_projeto, {}).get("valor", 1500.0)
    verba_mensal = float(dados.get("verba_mensal") or valor_padrao)
    meta_cpa = float(dados.get("meta_cpa", 15.0))
    logo_url = dados.get("logo_url", "").strip()
    cor_primaria = dados.get("cor_primaria", "#1E40AF").strip()
    whatsapp_grupo = dados.get("whatsapp_grupo", "").strip()
    ...
    with get_db() as conn:
        ...
        # Inserir cliente
        cursor.execute("""
        INSERT INTO clientes_trafego (
            nome_empresa, segmento, responsavel_nome, whatsapp, magic_token,
            verba_mensal, meta_cpa, status, data_inicio, logo_url, cor_primaria, plano_projeto, whatsapp_grupo
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, 'ativo', ?, ?, ?, ?, ?)
        """, (
            nome_empresa, segmento, responsavel_nome, whatsapp, magic_token,
            verba_mensal, meta_cpa, agora_iso, logo_url, cor_primaria, plano_projeto, whatsapp_grupo
        ))
        cliente_id = cursor.lastrowid

        # Mapeamento da fase por semana
        fases_map = {
            1: "Fase I – Piloto",
            2: "Fase II – Validação",
            3: "Fase III – Escala",
        ...
        # Inserir ciclo de 17 entregas do Roadmap
        for semana, titulo, freq, cor in ENTREGAS_TEMPLATE:
            fase = fases_map.get(semana, "Fase I – Piloto")
            cursor.execute("""
            INSERT INTO roadmap_entregas (cliente_id, semana, titulo, frequencia, frequencia_cor, status, projeto_tipo, fase_implementacao)
            VALUES (?, ?, ?, ?, ?, 'pendente', ?, ?)
            """, (cliente_id, semana, titulo, freq, cor, plano_projeto, fase))

        # Inserir métricas iniciais
        mes_atual = datetime.now().strftime("%B/%Y").capitalize()
        cursor.execute("""
        INSERT INTO metricas_trafego (
            cliente_id, mes_referencia, investimento_total, conversas_whatsapp,
            vendas_tintim, faturamento_rastreado, ultima_atualizacao
        )
        VALUES (?, ?, 0.0, 0, 0, 0.0, 'Início do Ciclo')
        """, (cliente_id, mes_atual))

        # Registrar atividade de boas-vindas
        cursor.execute("""
        INSERT INTO atividades_agencia (cliente_id, acao, categoria, data_hora)
        VALUES (?, ?, 'campanha', ?)
        """, (cliente_id, f"Cliente {nome_empresa} cadastrado no Propel CRM ({PROJETOS_ESCOPO.get(plano_projeto, {}).get('nome', 'Projeto')}) com Roadmap ativado.", agora_formatado))

        # Inserir template padrão de vitórias da semana
        for sem in [1, 2, 3, 4]:
            cursor.execute("""
            INSERT OR IGNORE INTO vitorias_semana_config (cliente_id, semana, saudacao, destaques_custom, proximos_passos)
            VALUES (?, ?, ?, ?, ?)
            """, (
                cliente_id, sem,
                f"Fala, {responsavel_nome}! Passando com o resumo oficial das nossas *Vitórias da Semana {sem}* na {nome_empresa}:",
                "Entregas cumpridas conforme o planejamento estratégico.",
                "Seguimos firmes nas otimizações e metas de conversão!"
            ))
...
def excluir_cliente_completo(cliente_id: int) -> bool:
    """
    Exclui um cliente de tráfego e todos os seus registros correlacionados de forma segura e em transação única:
    - roadmap_entregas
    - metricas_trafego
    - criativos_aprovacao
    - atividades_agencia
    - vitorias_semana_config
    - auditorias_digitais
    - clientes_trafego
    """
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, nome_empresa FROM clientes_trafego WHERE id = ?", (cliente_id,))
        cli = cursor.fetchone()
        if not cli:
            return False

        cursor.execute("DELETE FROM roadmap_entregas WHERE cliente_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM metricas_trafego WHERE cliente_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM criativos_aprovacao WHERE cliente_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM atividades_agencia WHERE cliente_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM vitorias_semana_config WHERE cliente_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM auditorias_digitais WHERE cliente_trafego_id = ?", (cliente_id,))
        cursor.execute("DELETE FROM clientes_trafego WHERE id = ?", (cliente_id,))
        conn.commit()
    return True


def adicionar_criativo_aprovacao(cliente_id: int, titulo: str, midia_url: str, formato: str, texto_copy: str, publico_alvo: str = "Público Segmentado") -> int:
    """Insere um novo criativo/anúncio na esteira de aprovação do cliente."""
    agora_iso = datetime.now().strftime("%Y-%m-%d %H:%M")
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute("""
        INSERT INTO criativos_aprovacao (
            cliente_id, titulo, midia_url, formato, texto_copy, publico_alvo, status, data_envio
        )
        VALUES (?, ?, ?, ?, ?, ?, 'pendente', ?)
        """, (cliente_id, titulo.strip(), midia_url.strip(), formato.strip(), texto_copy.strip(), publico_alvo.strip(), agora_iso))
        ...
        INSERT INTO atividades_agencia (cliente_id, acao, categoria, data_hora)
        VALUES (?, ?, 'criativo', ?)
        """, (cliente_id, f"Novo anúncio '{titulo}' enviado para aprovação no Portal do Cliente.", agora_iso))
        conn.commit()
        return cid
